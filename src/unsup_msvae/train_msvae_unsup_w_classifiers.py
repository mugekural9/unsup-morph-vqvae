...
from unsup.msvae_unsup_w_classifiers import MSVAE
...
argParser.add_argument("--klmax", help="datatype")
argParser.add_argument("--decnh", help="datatype")
argParser.add_argument("--zlemma", help="datatype")
argParser.add_argument("--ztag", help="datatype")
argParser.add_argument("--tau", help="datatype")
argParser.add_argument("--mintau", help="datatype")
argParser.add_argument("--bsize", help="datatype")
...
KL_MAX = float(args.klmax)
DEC_NH = int(args.decnh)

Z_LEMMA_NH =  int(args.zlemma)
Z_TAG_NH   =  int(args.ztag)
BATCHSIZE   =  int(args.bsize)

MIN_TAU     = float(args.mintau)
DEC_DROPOUT = 0.4
INPUT_EMB_DIM = 300
tau_str = args.tau
if tau_str == "hard":
...
TRN_ID = 'run'+str(RUNID)+"_"+str(NUM_CODEBOOK)+"x"+str(NUM_CODEBOOK_ENTRIES)+"_zLEM"+str(Z_LEMMA_NH)+"_zTAG"+ str(Z_TAG_NH)+ "_decnh"+str(DEC_NH)+"_klmax"+str(KL_MAX)+"_decdo"+str(DEC_DROPOUT)+"_inpemb"+str(INPUT_EMB_DIM)+"_bsize"+str(BATCHSIZE)+"_mintau"+str(MIN_TAU)+"_TAU_"+tau_str
...
if DATASET_TYPE == 'all':
    path = 'results/msved/unsup_w_classifiers/all/'+str(TRN_ID)
    ffile = '/kuacc/users/mugekural/workfolder/dev/git/unsup-morph-vqvae/src/dataset/tur.gold'
elif DATASET_TYPE == 'verbs':
    path = 'results/msved/unsup_w_classifiers/verbs/'+str(TRN_ID)
...
def get_temp(update_ind):
    return max(MIN_TAU, math.exp(-3 * 1e-5 * update_ind))
...
def config():
    ...
    for i in range(NUM_CODEBOOK):
        dictmeta.append(NUM_CODEBOOK_ENTRIES)
    ...
    args.ni = INPUT_EMB_DIM; 
    args.nz = Z_LEMMA_NH; 
    args.enc_nh = ENC_NH; args.dec_nh = DEC_NH
    args.dec_dropout_in = DEC_DROPOUT; 
    
    args.enc_dropout_in = 0.0; args.enc_dropout_out = 0.0
    args.device = 'cuda'
    args.update_temp = 2000

    model = MSVAE(args, trainset.charvocab, dictmeta, model_init, emb_init, NUM_CODEBOOK, NUM_CODEBOOK_ENTRIES, Z_TAG_NH, IS_HARD_TAU)
    model.to(device)
...
def train(model, opt, trnbatches, devbatches, tstbatches, charvocab, _trnbatches):
    ...
    for epc in range(EPOCHS):
        ...
        for bid in keys:
            ...
            kl_weight = get_kl_weight(update_ind, KL_MAX, 150000.0)
            update_ind +=1
            lx_src = tgt
            lx_tgt = tgt
            # (batchsize)
            loss_l, labeled_pred_loss, tag_correct, tag_total, labeled_recon_loss,  labeled_kl_loss, labeled_reinflect_recon_acc, preds, _ = model.loss_l(lx_src, tags, lx_tgt, kl_weight, tmp)
            #decoded_batches = decode_batch(lx_src, charvocab)
            #for p in range(len(preds)):
            #    selection = '-'.join([str(p) for p in preds[0].tolist()])
            #    word = decoded_batches[p]
            #    out_df.at[j, 'word'] = word
            #    out_df.at[j, 'selection'] = selection
            #    j+=1
        ...
        #out_df.to_csv('trn_selections_epc'+ str(epc)+'.csv')
        ## log            
        ##loss per token
        logging.info('epoch: %.1d, kl_weight: %.2f, tmp: %.2f' % (epc, kl_weight, tmp))
        logging.info('Trn--- loss: %.4f,  labeled_recon_loss: %.4f, labeled_kl_loss: %.4f, labeled_reinflect_recon_acc: %.4f' % (loss, labeled_recon_loss,  labeled_kl_loss,  labeled_reinflect_recon_acc))
        #-end of trn
        ...
        model.eval()
        selections = dict()
        exact_match_dev_acc = 0
        for bid, batch in devbatches.items():
            tgt,tags = batch
            lx_src = tgt
            lx_tgt = tgt
            loss, labeled_pred_loss, tag_correct, tag_total, labeled_recon_loss, labeled_kl_loss, labeled_reinflect_recon_acc, preds, recon_preds  = model.loss_l(lx_src, tags, lx_tgt, kl_weight, tmp, mode='val')
            decoded_batches = decode_batch(lx_src,      charvocab)
            pred_batches    = decode_batch(recon_preds, charvocab)

            for k in range(len(pred_batches)):
                if (decoded_batches[k][3:] == pred_batches[k]):
                    exact_match_dev_acc+=1
            ...
            for p in range(len(preds)):
                ...
                selections[selection].append(word)
            epoch_tag_correct += tag_correct
            epoch_tag_total_tokens += tag_total
            epoch_labeled_num_tokens +=  torch.sum(lx_tgt[:,1:] !=0).item()
        ...
        with open(path+'/dev_selections_epc'+str(epc)+'.json', 'w') as fp:
        ...
        labeled_kl_loss = epoch_labeled_kl_loss / numwords
        labeled_reinflect_recon_acc = epoch_labeled_reinflect_recon_acc / epoch_labeled_num_tokens
        logging.info('Tst--- loss: %.4f, labeled_recon_loss: %.4f, labeled_kl_loss: %.4f,  labeled_reinflect_recon_acc: %.4f, unique_tag_classes: %d, exact_match_dev_acc: %d' % ( loss,  labeled_recon_loss,  labeled_kl_loss, labeled_reinflect_recon_acc, len(selections), exact_match_dev_acc))
        ...
        for bid, batch in tstbatches.items():
            ...
            reinflected_form = model.reinflect(lemma, tgt, tmp)
            out_df.at[i, "REINF_wlemma"] = reinflected_form
            if gold_decoded == reinflected_form:
                exact_match_tst_reinflection_wlemma +=1
            i+=1
        logging.info("exact_match_tst_acc: %.3f" % (exact_match_tst_acc/tstsize))
        logging.info("exact_match_tst_reinflection_wlemma: %.3f" % (exact_match_tst_reinflection_wlemma/tstsize))
        out_df.to_csv(path+'/tst_results_epc'+str(epc)+'.csv')
        ...
        out_df = pd.DataFrame({})
        for i in range(50):
            out_df.at[i, "sampled"] = model.sample()
