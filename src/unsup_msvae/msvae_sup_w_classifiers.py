...
class MSVED_Decoder(nn.Module):
    ...
    def __init__(self, args, embed, charvocab, model_init, emb_init):
...
class MSVAE(nn.Module):
    def __init__(self, args, charvocab, dictmeta, model_init, emb_init):
        ...
        self.decoder = MSVED_Decoder(args, self.embed, charvocab, model_init, emb_init)
    ...
    def classifier_loss(self, enc_nh, tmp, tags):
        ...
        for i in range(len(self.classifiers)):
            ...
            xloss += loss(logits.squeeze(1), tags[:,i])
            preds.append(torch.argmax(sft(logits),dim=2))
            # (batchsize,tagvocabsize)
            gumbel_logits = F.gumbel_softmax(logits, tau=tmp, hard=True).squeeze(1)
        ...
        for i in range(tags.shape[1]):
            tag_correct +=  (preds[i].squeeze(1) == tags[:,i]).sum().item()
    ...
    def labeled_msved_loss(self, x, tags, reinflect_surf, kl_weight, tmp, mode='train'):
        ...
        gumbel_tag_embeddings, xloss, tag_correct, tag_total = self.classifier_loss(encoder_fhs, tmp, tags)
        if mode == 'train':
            # (batchsize, 1, nz)
        ...
        for i in range(tags.shape[1]):
            ...
            #(batchsize,200)
            embed = ((tags[:,i]!=0).repeat(200,1).t() * tag_emb)
            embeds.append(embed)
        # (batchsize, 11, 200)
        ...
        # (batchsize, 1, 11)
        tag_attention_masks = torch.stack(tag_attention_masks).t().unsqueeze(1)
        # (batchsize, 1, 200)
        #tag_all_embed = torch.sum(tag_embeddings,dim=1).unsqueeze(1)
        tag_all_embed = torch.sum(embeds, dim=1).unsqueeze(1)
        #TODO: add bias
    ...
    def decode(self, x, tags, tmp):
        ...
        while True:
            decoder_output, decoder_hidden, decoder_attention = self.decoder(
                decoder_input, z, decoder_hidden, tag_embeddings, tag_attention_masks)
            output_logits.append(decoder_output)
            topv, topi = decoder_output.topk(1)
            decoder_input = topi.squeeze(1).detach()  # detach from history as input
