...
from unsup.msvae_sup import MSVAE
...
def config():
    logging.basicConfig(handlers=[
            logging.FileHandler(path+"/training_msved.log"),
    ...
    model = MSVED(args, trainset.charvocab, dictmeta, model_init, emb_init)
    model.to(device)
...
def train(model, opt, trnbatches, devbatches, tstbatches, charvocab, _trnbatches):
    ...
    for epc in range(EPOCHS):
        ...
        for bid, batch in tstbatches.items():
            ...
            ## Copy the tgt
            reinflected_form = model.decode(tgt,tags)
