...
class VQVAE(nn.Module):
    def __init__(self, vocabsize, dictmeta, ENC_NH, DEC_NH, Z_LEMMA_NH, Z_TAG_NH, DEC_DROPOUT_IN, INPUT_EMB_DIM, NUM_CODEBOOK, NUM_CODEBOOK_ENTRIES, BIDIRECTIONAL):
        ...
        for i in range(NUM_CODEBOOK):
            if self.encoder.bidirectional:
                self.z_linears.append(nn.Linear(ENC_NH, Z_TAG_NH))
    ...
    def forward(self, x: torch.Tensor, entries: torch.Tensor, mode='dict-unsupervised'):
        out, out_h, fwd, bck  = self.encoder(x)
        #(1,batchsize,zdim)
        mu     = self.z_lemma_mu(bck) 
        logvar = self.z_lemma_logvar(bck) 
        ...
        Q = torch.sum(qloss)
        return totalloss, nonpadded_tokens, pred_tokens, Q, KL, quantized_indices, (lemma,tmp_quantized_z), q_selections, quantized_codes

    
    def decode(self, x: torch.Tensor, entries: torch.Tensor, mode='dict-unsupervised'):
        out, out_h, fwd, bck  = self.encoder(x)
        #(1,batchsize,zdim)
        mu     = self.z_lemma_mu(bck) 
        logvar = self.z_lemma_logvar(bck) 
