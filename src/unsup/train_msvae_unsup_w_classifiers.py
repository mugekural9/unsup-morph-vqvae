...
def config():
    ...
    model = MSVAE(args, trainset.charvocab, dictmeta, model_init, emb_init, NUM_CODEBOOK, NUM_CODEBOOK_ENTRIES)
    model.to(device)
...
def train(model, opt, trnbatches, devbatches, tstbatches, charvocab, _trnbatches):
    ...
    for epc in range(EPOCHS):
        ...
        labeled_kl_loss = epoch_labeled_kl_loss / numwords
        labeled_reinflect_recon_acc = epoch_labeled_reinflect_recon_acc / epoch_labeled_num_tokens
        logging.info('Tst--- loss: %.4f, labeled_recon_loss: %.4f, labeled_kl_loss: %.4f,  labeled_reinflect_recon_acc: %.4f' % ( loss,  labeled_recon_loss,  labeled_kl_loss, labeled_reinflect_recon_acc))
