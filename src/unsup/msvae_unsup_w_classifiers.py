...
class MSVED_Decoder(nn.Module):
    ...
    def __init__(self, args, embed, charvocab, model_init, emb_init, NUM_CODEBOOK, NUM_CODEBOOK_ENTRIES):
        ...
        self.attn = nn.Linear(300+ 150+ 256, NUM_CODEBOOK)
...
class MSVAE(nn.Module):
    def __init__(self, args, charvocab, dictmeta, model_init, emb_init, NUM_CODEBOOK, NUM_CODEBOOK_ENTRIES):
        ...
        self.decoder = MSVED_Decoder(args, self.embed, charvocab, model_init, emb_init, NUM_CODEBOOK, NUM_CODEBOOK_ENTRIES)
    ...
    def sample(self):
        ...
        for i in range(self.NUM_CODEBOOK):
            #(batchsize, 200)
            tag_emb = self.tag_embeddings[i](torch.tensor(2).to('cuda'))
            tag_embeds.append(tag_emb)
        # (batchsize, 11, 200)
        tag_embeddings = torch.stack(tag_embeds,dim=1)
        # (batchsize, 1, 200)
