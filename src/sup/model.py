...
class VQVAE(nn.Module):
    def __init__(self, vocabsize, dictmeta, ENC_NH, DEC_NH, Z_NH, DEC_DROPOUT_IN, INPUT_EMB_DIM):
        nn.Module.__init__(self)
        self.encoder = Encoder_GRU_VQVAE(vocabsize, ENC_NH, Z_NH, INPUT_EMB_DIM)
        ...
        for i in dictmeta:
            if self.encoder.bidirectional:
                ...
                self.z_lemma_mu = nn.Linear(ENC_NH*2, Z_NH)
                self.z_lemma_logvar = nn.Linear(ENC_NH*2, Z_NH)
            else:
                self.z_linears.append(nn.Linear(ENC_NH, Z_NH))
                self.z_lemma_mu = nn.Linear(ENC_NH, Z_NH)
                self.z_lemma_logvar = nn.Linear(ENC_NH*2, Z_NH)

            self.quantizers.append(Quantizer(Z_NH, i))  

        self.up_from_z_layer = nn.Linear(Z_NH, DEC_NH)
        self.decoder = Decoder_GRU_VQVAE(vocabsize, DEC_NH, Z_NH, DEC_DROPOUT_IN, INPUT_EMB_DIM)

        loc   = torch.zeros(Z_NH)
        scale = torch.ones(Z_NH)
        self.prior = torch.distributions.normal.Normal(loc, scale)

    def forward(self, x: torch.Tensor, entries: torch.Tensor, mode='dict-supervised'):
        ...
        for idx,quantizer in enumerate(self.quantizers):
            ...
            _quantized_z, _quantized_indices, _qloss, true_selection, total_selection = quantizer(z,  entries[:,idx], mode)
            q_selections[idx] = (true_selection, total_selection)
        ...
        Q = torch.sum(qloss)
        return totalloss, nonpadded_tokens, pred_tokens, Q, KL, quantized_indices, (lemma,tmp_quantized_z), q_selections

    
    def decode(self, x: torch.Tensor, entries: torch.Tensor, mode='dict-supervised'):
        out, out_h  = self.encoder(x)
        #(1,batchsize,zdim)
        mu     = self.z_lemma_mu(out_h) 
        logvar = self.z_lemma_logvar(out_h) 
    ...
    def sample(self, entries=None, mode='dict-supervised'):
        #(z:1,1,zdim) 
        ...
        for idx,quantizer in enumerate(self.quantizers):
            if entries is not None:
                _quantized_z, _quantized_indices = quantizer.sample(torch.tensor(entries[idx]))
            else:
    ...
    def reinflect(self, x, entries, mode='dict-supervised'):
        out, out_h  = self.encoder(x)
        #(1,batchsize,zdim)
        lemma     = self.z_lemma_mu(out_h) 
        quantized_z = []
        quantized_indices = []
        for idx,quantizer in enumerate(self.quantizers):
            #(quantized: 1,batchsize,dec_nh),
...
class Encoder_GRU_VQVAE(nn.Module):
    def __init__(self, vocabsize, ENC_NH, Z_NH, INPUT_EMB_DIM):
        nn.Module.__init__(self)
...
class Quantizer(nn.Module):
    ...
    def forward(self, h: torch.Tensor, entry: torch.Tensor, mode):
        ...
        true_selection = sum(closest.squeeze(1) == entry).item()
        total_selection = len(entry)

        ## Change this for SEMI-SUP!
        if mode == 'dict-supervised':
        ...
        return quantized, quantized_indices, loss, true_selection, total_selection
...
class Decoder_GRU_VQVAE(nn.Module):
    def __init__(self, vocabsize, DEC_NH, Z_NH, DEC_DROPOUT_IN, INPUT_EMB_DIM):
        nn.Module.__init__(self)
        self.input_dim = INPUT_EMB_DIM
