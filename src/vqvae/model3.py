...
class VQVAE3(nn.Module):
    def __init__(self):
        ...
        self.encoder = Encoder_GRU_VQVAE3()
        quantizers = nn.ModuleList()
        for i in range(N):
            quantizers.append(Quantizer3())
        self.quantizers = quantizers
        self.decoder = Decoder_GRU_VQVAE3()
    ...
    def forward(self, x: torch.Tensor):
        ...
        for i in range(N):
            _z = z[:,:,i*perz:(i*perz)+perz]
            quantized, quantized_indices, qloss = self.quantizers[i](_z)
            quantizeds[i] = quantized
            quantized_idx[i] = quantized_indices
            qlosses[i] = qloss

        quantized = torch.cat([val for val in quantizeds.values()],dim=2)
        hidden=quantized
        ...
        # 32 meann for codebooks
        qloss = torch.mean(torch.stack([val for val in qlosses.values()]),dim=0)
        
        # sum Q loss over batch (per word)
        Q = torch.sum(qloss)
        return totalloss, nonpadded_tokens, pred_tokens, Q, quantized_idx
    ...
    def random_entry(self, x: torch.Tensor, entry):
        ...
        quantized0 = self.quantizers[0].embeddings.weight[entry].unsqueeze(0).unsqueeze(0)
        quantized1 = self.quantizers[1].embeddings.weight[entry].unsqueeze(0).unsqueeze(0)

        quantized = torch.cat((quantized0,quantized1),dim=2)
...
class Encoder_GRU_VQVAE3(nn.Module):
    def __init__(self):
        nn.Module.__init__(self)
...
class Decoder_GRU_VQVAE3(nn.Module):
    def __init__(self):
        nn.Module.__init__(self)
        self.input_dim = INPUT_EMB_DIM
