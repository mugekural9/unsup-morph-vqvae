...
from vqvae.model2 import VQVAE2
...
def train(model, opt, trnbatches, devbatches, tstbatches, charvocab):
    ...
    for epc in range(EPOCHS):
        ...
        for bid in keys:
            ...
            opt.zero_grad()
            batchloss, nonpadded_tokens, predtokens, KL, Q, quantized_indices = model(batch)
            #optimize for per token nll + KL per word
            optloss = (batchloss/nonpadded_tokens) + (kl_weight * (KL/batch.size(0))) + (Q/batch.size(0))
            optloss.backward()
            opt.step()
        ...
        for bid, batch in tstbatches.items():
            _, _, _, _, _, quantized_indices = model(batch)
            if k <30:
                for j in range(10):
