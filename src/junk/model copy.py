...
class VQVAE(nn.Module):
    ...
    def forward(self, x: torch.Tensor, entries: torch.Tensor):
        ...
        for idx,quantizer in enumerate(self.quantizers):
            ...
            _quantized_z, _quantized_indices, _qloss = quantizer(z,  entries[:,idx])
    ...
    def decode(self, x: torch.Tensor, entries: torch.Tensor):
        out, out_h  = self.encoder(x)
        #(1,batchsize,zdim)
        lemma = self.z_lemma(out_h) 
        quantized_z = []
