...
from vqvae.model_4 import VQVAE
...
def config():
    ...
    zdict_affix_1 = torch.load('results/ae/1/10trnwords_z.pt')
    ...
    for idx, tensor in zdict_affix_1.items():
        model.quantizer_affix_1.embeddings.weight.data[idx] =  tensor
        model.quantizer_affix_2.embeddings.weight.data[idx] =  tensor
...
def train(model, opt, trnbatches, devbatches, tstbatches, charvocab):
    ...
    for epc in range(EPOCHS):
        ...
        epc_q_indices_affix_1 = dict({'trn': defaultdict(lambda:0), 'dev':defaultdict(lambda:0), 'tst':defaultdict(lambda:0)})
        epc_q_indices_affix_2 = dict({'trn': defaultdict(lambda:0), 'dev':defaultdict(lambda:0), 'tst':defaultdict(lambda:0)})
        ...
        for bid in keys:
            ...
            Q_lemma, Q_affix = Q
            quantized_indices_lemma, quantized_indices_affix_1, quantized_indices_affix_2 = quantized_indices
            ...
            qlist_affix_1 = quantized_indices_affix_1.squeeze(1).tolist()
            qlist_affix_2 = quantized_indices_affix_2.squeeze(1).tolist()

            for q in set(qlist_lemma):
                epc_q_indices_lemma['trn'][q] +=qlist_lemma.count(q)
            for q in set(qlist_affix_1):
                epc_q_indices_affix_1['trn'][q] +=qlist_affix_1.count(q)
            for q in set(qlist_affix_2):
                epc_q_indices_affix_2['trn'][q] +=qlist_affix_2.count(q)
        ...
        for bid, batch in devbatches.items():
            ...
            for q in set(qlist_affix_1):
                epc_q_indices_affix_1['dev'][q] +=qlist_affix_1.count(q)
            for q in set(qlist_affix_2):
                epc_q_indices_affix_2['dev'][q] +=qlist_affix_2.count(q)
        ...
        for bid, batch in tstbatches.items():
            ...
            for q in set(qlist_affix_1):
                epc_q_indices_affix_1['tst'][q] +=qlist_affix_1.count(q)
            for q in set(qlist_affix_2):
                epc_q_indices_affix_2['tst'][q] +=qlist_affix_2.count(q)
        ...
        logging.info("    || Num_Q_indices_affix_1: %s" % len(epc_q_indices_affix_1['trn']))
        logging.info("    || Num_Q_indices_affix_2: %s" % len(epc_q_indices_affix_2['trn']))
        ...
        logging.info("    || Num_Q_indices_affix_1: %s" % len(epc_q_indices_affix_1['dev']))
        logging.info("    || Num_Q_indices_affix_2: %s" % len(epc_q_indices_affix_2['dev']))
        ...
        logging.info("    || Num_Q_indices_affix_1: %s" % len(epc_q_indices_affix_1['tst']))
        logging.info("    || Num_Q_indices_affix_2: %s" % len(epc_q_indices_affix_2['tst']))
        ...
        writer.add_scalar('loss/tst',   epc_loss['dev']/ epc_nonpaddedtokens['tst'], epc)
        ...
        writer.add_scalar('Q_NUM_affix_1/trn',   len(epc_q_indices_affix_1['trn']), epc)
        writer.add_scalar('Q_NUM_affix_1/dev',   len(epc_q_indices_affix_1['dev']), epc)
        writer.add_scalar('Q_NUM_affix_1/tst',   len(epc_q_indices_affix_1['tst']), epc)

        writer.add_scalar('Q_NUM_affix_2/trn',   len(epc_q_indices_affix_2['trn']), epc)
        writer.add_scalar('Q_NUM_affix_2/dev',   len(epc_q_indices_affix_2['dev']), epc)
        writer.add_scalar('Q_NUM_affix_2/tst',   len(epc_q_indices_affix_2['tst']), epc)
