## A model with two codebooks with assumption that lemma and affixes will be discretized seperately into them.
...
class VQVAE(nn.Module):
    def __init__(self):
        ...
        self.linear_lemma = nn.Linear(Z_NH, Z_NH)
        self.linear_affix = nn.Linear(Z_NH, Z_NH)

        self.quantizer_lemma = Quantizer(NUM_ENTRY_LEMMA)
        self.quantizer_affix = Quantizer(NUM_ENTRY_AFFIX)
    ...
    def forward(self, x: torch.Tensor):
        ...
        z_lemma = self.linear_lemma(z)
        z_affix = self.linear_affix(z)

        #(quantized: 1,batchsize,dec_nh), (loss: batchsize)
        quantized_z_lemma, quantized_indices_lemma, qloss_lemma = self.quantizer_lemma(z_lemma)
        quantized_z_affix, quantized_indices_affix, qloss_affix = self.quantizer_affix(z_affix)
        quantized_z = torch.add(quantized_z_lemma, quantized_z_affix)
        ...
        Q_lemma = torch.sum(qloss_lemma)
        Q_affix = torch.sum(qloss_affix)

        return totalloss, nonpadded_tokens, pred_tokens, (Q_lemma, Q_affix), (quantized_indices_lemma, quantized_indices_affix)
    ...
    def decode(self, x: torch.Tensor):
        ...
        quantized_z = quantized_z_lemma
        hidden = self.up_from_z_layer(quantized_z)
        quantized_z = torch.permute(quantized_z, (1,0,2)) 
        pred_tokens_only_lemma = self.decoder.decode(x, hidden, quantized_z, MAX_LENGTH)


        quantized_z = quantized_z_affix
        hidden = self.up_from_z_layer(quantized_z)
        quantized_z = torch.permute(quantized_z, (1,0,2)) 
        pred_tokens_only_affix = self.decoder.decode(x, hidden, quantized_z, MAX_LENGTH)

        return pred_tokens, pred_tokens_only_lemma, pred_tokens_only_affix


    def reinflect(self, x, lemma_idx, affix_idx):
        quantized_z_lemma = self.quantizer_lemma.embeddings.weight[lemma_idx].unsqueeze(0).unsqueeze(0)
        quantized_z_affix = self.quantizer_affix.embeddings.weight[affix_idx].unsqueeze(0).unsqueeze(0)
        quantized_z = torch.add(quantized_z_lemma, quantized_z_affix)
        hidden = self.up_from_z_layer(quantized_z)
        quantized_z = torch.permute(quantized_z, (1,0,2)) 
        pred_tokens = self.decoder.decode(x, hidden, quantized_z, MAX_LENGTH)
