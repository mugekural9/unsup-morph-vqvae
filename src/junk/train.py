...
DEC_DROPOUT = 0.2
INPUT_EMB_DIM = 128
TRN_ID = "LEMMAS_INC_SIL_v2_BI"+str(BIDIRECTIONAL)+"_"+str(NUM_CODEBOOK)+"x"+str(NUM_CODEBOOK_ENTRIES)+"_UtrNOUN_zLEM"+str(Z_LEMMA_NH)+"_zTAG"+ str(Z_TAG_NH)+ "_decnh"+str(DEC_NH)+"_kl"+str(KL_WEIGHT)+"_epc"+str(KL_ANNEAL_EPC)+"_strt"+str(KL_START_EPC)+"_decdo"+str(DEC_DROPOUT)+"_inpemb"+str(INPUT_EMB_DIM)+"_bsize"+str(BATCHSIZE)
...
def config():
    ...
    trainset = MorphDataset("/kuacc/users/mugekural/workfolder/dev/git/unsup-morph-vqvae/src/dataset/tur/train_tur_filtered_duplicates_merged_stdata_NOUNS_shuffled")
...
def train(model, opt, trnbatches, devbatches, tstbatches, charvocab, _trnbatches):
    ...
    for epc in range(EPOCHS):
        ...
        ## Copy tst with one-step at a time
        out_df = pd.DataFrame({})
        ...
        for bid, batch in tstbatches.items():
            tgt,_,lemma = batch
            predtokens, quantized_indices = model.decode(tgt)
        ...
        exact_match_tst_reinflection_wlemma_TAGFINDER = 0
        out_df = pd.DataFrame({})
        i=0
        #refresh dict
        for k,v in tags_2_entries.items():
            tags_2_entries[k] = defaultdict(lambda:0)
        for bid, batch in tstbatches.items():
            tgt,_,lemma = batch
            predtokens, quantized_indices = model.reinflect(lemma,tgt)
            lemmas = decode_batch(lemma[:,1:], charvocab)
            ...
            for g,p,l in zip(gold_decoded_batches, pred_decoded_batches, lemmas):
                out_df.at[i, "gold"] = g
                out_df.at[i, "lemma"] = l
                key = ';'.join([str(qi.item()) for qi in quantized_indices])
                out_df.at[i, "selected_indices"] = key
                if p == g:
                ...
                GOLD_TAG = word_tags[g[:-4]]
                FOUND_TAG = max(tags_2_entries[GOLD_TAG])
                FOUND_TAG = [torch.tensor(int(i)) for i in FOUND_TAG.split(';')]
                predtokens2, quantized_indices2 = model.reinflect(lemma, tgt, FOUND_TAG)
                out_df.at[i, "pred"] = p
                out_df.at[i, "pred_TAGFINDER"] = decode_batch(predtokens2, charvocab)[0]
                out_df.at[i, "selected_indices_TAGFINDER"] = ';'.join([str(qi.item()) for qi in quantized_indices2])
                out_df.at[i, "IS_EQUAL"] = (decode_batch(predtokens2, charvocab)[0] == p)

                if decode_batch(predtokens2, charvocab)[0] == g:
                    exact_match_tst_reinflection_wlemma_TAGFINDER +=1
                    out_df.at[i, "exact_match_TAGFINDER"] = 1
                else:
                    out_df.at[i, "exact_match_TAGFINDER"] = 0
                i+=1
            #if bid == 200:
            #    break
        out_df.to_csv(path+'/tst_reinflections_'+str(epc)+'.csv')
        logging.info('exact_match_tst_reinflection_wlemma: %.3f'           % (exact_match_tst_reinflection_wlemma/i))
        logging.info('exact_match_tst_reinflection_wlemma_TAGFINDER: %.3f' % (exact_match_tst_reinflection_wlemma_TAGFINDER/i))

        writer.add_scalar('reinflection_exact_match', (exact_match_tst_reinflection_wlemma/i), epc)
