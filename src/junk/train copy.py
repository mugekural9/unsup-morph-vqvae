...
def config():
    ...
    trainset = MorphDataset("/kuacc/users/mugekural/workfolder/dev/tur/tur")
    devset = MorphDataset("/kuacc/users/mugekural/workfolder/dev/tur/tur", charvocab=trainset.charvocab, tagsvocab=trainset.tagsvocab)
...
def train(model, opt, trnbatches, devbatches, tstbatches, charvocab):
    ...
    for epc in range(EPOCHS):
        ...
        for bid in keys:
            ...
            tgt,tags = batch
            opt.zero_grad()
            batchloss, nonpadded_tokens, predtokens, Q, quantized_indices = model(tgt, tags)
        ...
        for bid, batch in tstbatches.items():
            tgt,tags = batch
            _, _, _, _,  quantized_indices = model(tgt,tags)
